...
export function MetricsSection() {
  const [metricsData, setMetricsData] = useState([
    { 
      ...
      suffix: "", 
      prefix: "",
      label: "Agent decisions evaluated today",
    ...
    { 
      ...
      suffix: ".4%", 
      prefix: "",
      label: "Safe completion rate",
      sublabel: "adhering to policy boundaries",
    },
    { 
      value: 37, 
      suffix: "", 
      ...
      sublabel: "before real-world user impact",
    },
  ]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setMetricsData([
            { 
              value: data.decisions_evaluated || 12847, 
              suffix: "", 
              prefix: "",
              label: "Agent decisions evaluated today",
              sublabel: data.live_evaluations_count > 0 
                ? `includes ${data.live_evaluations_count} live evaluated`
                : "across all connected platforms",
            },
            { 
              value: data.safe_completion_rate !== undefined ? Math.floor(data.safe_completion_rate) : 98, 
              suffix: data.safe_completion_rate !== undefined ? `.${Math.round((data.safe_completion_rate % 1) * 10)}%` : ".4%", 
              prefix: "",
              label: "Safe completion rate",
              sublabel: "adhering to policy boundaries",
            },
            { 
              value: data.mistakes_caught || 37, 
              suffix: "", 
              prefix: "",
              label: "Mistakes caught",
              sublabel: data.live_incidents_count > 0
                ? `includes ${data.live_incidents_count} live incidents`
                : "before real-world user impact",
            },
          ]);
        }
      } catch {
    ...
    fetchStats();
    const t = setInterval(fetchStats, 10000); // Poll every 10 seconds
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              <span className="flex items-center gap-2 px-3 py-1 bg-[#8f9cff]/10 text-[#8f9cff] text-xs font-mono">
                <span className="w-2 h-2 rounded-full bg-[#8f9cff] animate-pulse" />
                LIVE
              </span>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="text-4xl md:text-5xl lg:text-6xl font-display tracking-tight mb-4 whitespace-nowrap overflow-hidden">
              <AnimatedNumber end={metricsData[0].value} suffix={metricsData[0].suffix} prefix={metricsData[0].prefix} />
            </div>
            <div className="mb-6">
              <DotGraph color="white" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
            <div className="text-lg text-foreground mb-2">{metricsData[0].label}</div>
            <div className="text-sm text-muted-foreground font-mono">{metricsData[0].sublabel}</div>
          </div>
