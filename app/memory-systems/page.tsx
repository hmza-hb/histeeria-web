  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none opacity-[0.07]">
        {[...Array(10)].map((_, i) => <div key={`h${i}`} className="absolute h-px bg-white w-full" style={{ top: `${10 * (i + 1)}%` }} />)}
        {[...Array(14)].map((_, i) => <div key={`v${i}`} className="absolute w-px bg-white h-full" style={{ left: `${7.14 * (i + 1)}%` }} />)}
      </div>
      ...
      <section className="py-32 px-6 lg:px-12 relative z-10">
        <div ref={archRef.ref} className="max-w-[1400px] mx-auto">
          ...
          {/* Image strip */}
          <div className={`mt-26 grid grid-cols-2 md:grid-cols-1 gap-4 transition-all duration-1000 delay-300 ${archRef.visible ? "opacity-100" : "opacity-0"}`}>
